		...
		const options: ChartOptions<"doughnut"> = {
			...
			plugins: {
				...
				tooltip: {
					...
					callbacks: {
						label: (context) =>
							`${context.label}: ${context.raw} (${total > 0
									...
									{statusEntries.length > 1 && (
										<div className={styles.carouselIndicators}>
											{statusEntries.map((_, index) => (
												<div
													key={`indicator-${index}`}
													className={`${styles.carouselDot} ${index === currentStatusIndex ? styles.active : ""
														}`}
...
export default function StatisticsClient(props: StatisticsClientProps) {
	return (
