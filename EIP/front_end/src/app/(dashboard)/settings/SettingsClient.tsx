...
function SettingsContent({
	...
	const saveGoalSettings = async (e: React.FormEvent) => {
		...
		await toast.promise(savePromise, {
			...
			error: (err: unknown) => {
				setSaving(false);
				const msg = err instanceof Error ? err.message : (t("settings.errorSave") as string);
				return msg;
			},
		});
	};
	...
	const handleDisconnectAllServices = async () => {
		...
		await toast.promise(disconnectPromise, {
			...
			error: (err: unknown) => {
				setIsDisconnecting(false);
				const msg = err instanceof Error ? err.message : (t("settings.tryAgain") as string);
				return `${t("settings.errorDisconnect") as string}: ${msg}`;
			},
		});
	};
