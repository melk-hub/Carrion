...
function OrganizationSettingsContent() {
	...
	useEffect(() => {
		if (isInviteModalOpen && isTeacher) {
			setNewInviteRole("STUDENT");
		}
	}, [isInviteModalOpen, isTeacher]);

	const totalPages = Math.ceil(invitations.length / ITEMS_PER_PAGE);
	const paginatedInvitations = useMemo(() => {
		const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
		return invitations.slice(startIndex, startIndex + ITEMS_PER_PAGE);
	...
	const handleConfirmRevoke = async () => {
		...
		try {
			...
			const updatedList = invitations.filter(i => i.id !== revokingInvite.id);
			setInvitations(updatedList);

			if (updatedList.length <= (currentPage - 1) * ITEMS_PER_PAGE && currentPage > 1) {
	...
	const canActOnInvite = (inviteRole: string) => {
		if (isOwner) return true;
		if (isTeacher && inviteRole === "STUDENT") return true;
		return false;
	};
								...
								<table className={styles.table}>
									...
									<tbody>
										{paginatedInvitations.map((invite) => {
											const showActions = canActOnInvite(invite.role as string);

											return (
												<tr key={invite.id}>
													<td data-label={t("organization.table.email")}>
														<span className={styles.cellContent} style={{ fontWeight: 600 }}>{invite.email}</span>
													</td>
													<td data-label={t("organization.table.role")}>
														<span className={`${styles.roleBadge} ${getRoleBadgeClass(invite.role)}`}>
															{invite.role}
														</span>
													</td>
													<td data-label={t("organization.table.invitedBy")}>
														<span className={styles.cellContent} style={{ color: '#6b7280', fontSize: '0.85rem' }}>{invite.inviter.email}</span>
													</td>
													<td data-label={t("organization.table.expires")}>
														<span className={styles.cellContent} style={{ color: '#6b7280', fontSize: '0.85rem' }}>{new Date(invite.expiresAt).toLocaleDateString()}</span>
													</td>
													<td className={styles.actionsCellMobile}>
					...
					<div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
						...
						<div className={styles.formGroup}>
							...
							<select
								...
								disabled={isTeacher}
								style={isTeacher ? { backgroundColor: '#f3f4f6', cursor: 'not-allowed' } : {}}
							>
