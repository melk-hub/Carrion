...
interface OrganizationData {
	organization: { id: string; name: string; organizationInvitations: Invitation[] };
...
function OrganizationContent() {
	...
	const { organizationMemberInfo, isAuthenticated } = useAuth();
	...
	useEffect(() => {
		const fetchOrganizationData = async () => {
			try {
				if (
					isAuthenticated &&
					organizationMemberInfo?.userRole &&
					organizationMemberInfo?.organizationId
				) {
	...
	}, [isAuthenticated, organizationMemberInfo]);
	...
	const handleManageClick = () => router.push("/organization/settings");

	const handleEditClick = (member: OrganizationMember) => {
		setEditingMember(member);
		const currentRole = ["TEACHER", "STUDENT"].includes(member.userRole as unknown as string) ? member.userRole : "STUDENT";
	...
	const handleSaveRole = async () => {
		...
		try {
			...
			if (data) {
				const updatedMembers = data.members.map(m => m.id === editingMember.id ? { ...m, userRole: selectedRole as unknown as OrganizationRole } : m);
	...
	const handleKickClick = (member: OrganizationMember) => setKickingMember(member);

	const handleConfirmKick = async () => {
		if (!kickingMember) return;
		setIsSaving(true);
		try {
			await ApiService.delete('/organization/kick-member', { memberId: kickingMember.user.id, organizationId: organizationMemberInfo?.organizationId });
			if (data) {
	...
	const handleViewMember = (member: OrganizationMember) => setViewingMember(member);
	const handleBack = () => setViewingMember(null);
		...
		return (
			<main className={styles.container}>
				<header className={styles.viewAsHeader}>
					...
					<button onClick={handleBack} className={styles.backButton} aria-label={t("common.back") as string}>{t("common.back")}</button>
				</header>
				<section className={styles.viewAsContainer}>
					<div style={{ color: '#64748b', textAlign: 'center', padding: '2rem' }}>{t("organization.viewAs.placeholder", { name: getDisplayName(viewingMember.user) })}</div>
				</section>
			</main>
		);
	}
		...
		<main className={styles.container}>
			...
			<header className={styles.organizationHeader}>
				...
				<div className={styles.headerActions}>
					<button className={styles.actionButton} onClick={handleManageClick} title={t("organization.settings.title") as string}>
			...
			<section className={styles.statsGrid} aria-label={t("organization.stats.label") as string}>
				...
				<div className={styles.statCard}>
					<div className={styles.statHeader}>
						...
						<div className={styles.infoIcon}>i<span className={styles.tooltipText}>{t("organization.stats.applications.tooltip")}</span></div>
					</div>
				...
				<div className={styles.statCard}>
					<div className={styles.statHeader}>
						...
						<div className={styles.infoIcon}>i<span className={styles.tooltipText}>{t("organization.stats.activity.tooltip")}</span></div>
					</div>
					<span className={styles.statValue} style={{ color: activityRate >= 50 ? '#10b981' : '#f59e0b' }}>{activityRate}%</span>
			...
			<section className={styles.membersContainer}>
				<div className={styles.sectionHeader}><h2 className={styles.sectionTitle}>{t("organization.table.title")}</h2></div>
									...
									<tr key={member.id}>
										<td>
											<div className={styles.userCell}>
												<div className={styles.avatarPlaceholder} aria-hidden="true">{getDisplayName(member.user).charAt(0).toUpperCase()}</div>
												<span className={styles.userName}>{getDisplayName(member.user)}</span>
											</div>
										</td>
										<td style={{ color: "#64748b" }}>{member.user.email}</td>
										<td><span className={`${styles.roleBadge} ${getRoleBadgeClass(member.userRole as unknown as string)}`}>{member.userRole}</span></td>
										<td>
										...
										{canManage && (
											<td style={{ textAlign: "right" }}>
												<div className={styles.actionsCell}>
													<button className={styles.iconButton} onClick={() => handleViewMember(member)} title={t("common.view") as string} aria-label={t("organization.actions.viewUser", { name: getDisplayName(member.user) }) as string}>
														<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
													</button>
													{isEditable && (
														<>
															<button className={styles.iconButton} title={t("common.edit") as string} onClick={() => handleEditClick(member)} aria-label={t("organization.actions.editRole", { name: getDisplayName(member.user) }) as string}>
																<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
															</button>
															<button className={`${styles.iconButton} ${styles.deleteBtn}`} title={t("common.delete") as string} onClick={() => handleKickClick(member)} aria-label={t("organization.actions.kickUser", { name: getDisplayName(member.user) }) as string}>
			...
			{editingMember && (
				<div className={styles.modalOverlay} onClick={handleCloseModal}>
					<div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
						...
						<p className={styles.modalDescription}>{t("organization.modals.editRole.description", { name: getDisplayName(editingMember.user) })}</p>
						<div className={styles.formGroup}>
							<label htmlFor="role-select" className={styles.label}>{t("organization.modals.editRole.newRole")}</label>
							<select id="role-select" className={styles.selectInput} value={selectedRole} onChange={(e) => setSelectedRole(e.target.value)}>
								<option value="STUDENT">STUDENT</option>
								<option value="TEACHER">TEACHER</option>
							</select>
						</div>
						<div className={styles.modalActions}>
							<button className={styles.cancelButton} onClick={handleCloseModal} disabled={isSaving}>{t("common.cancel")}</button>
							<button className={styles.saveButton} onClick={handleSaveRole} disabled={isSaving}>{isSaving ? t("common.saving") : t("common.save")}</button>
						</div>
					</div>
				</div>
			)}
			{kickingMember && (
				<div className={styles.modalOverlay} onClick={handleCloseModal}>
					<div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
						...
						<p className={styles.modalDescription}>{t("organization.modals.kick.description", { name: getDisplayName(kickingMember.user) })}</p>
						<div className={styles.modalActions}>
							<button className={styles.cancelButton} onClick={handleCloseModal} disabled={isSaving}>{t("common.cancel")}</button>
							<button className={styles.dangerButton} onClick={handleConfirmKick} disabled={isSaving}>{isSaving ? t("common.processing") : t("organization.modals.kick.confirmButton")}</button>
						</div>
					</div>
				</div>
			)}
